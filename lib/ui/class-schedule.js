...
export function openScheduleEditor(ouId, anchor, courseLabel, options = {}) {
  ...
  editorState.draft = saved
    ...
    : { link: '', blocks: [{ days: options.day !== undefined ? [options.day] : [], time: options.time || DEFAULT_TIME, duration: DEFAULT_DURATION }] };
  ...
  return `
    ...
    <div class="cs-row">
      <label class="cs-field">
        ...
        <div class="cs-time-split">
          <input class="cs-input" type="number" min="0" max="23" data-cs-hour value="${(block.time || DEFAULT_TIME).split(':')[0]}" ${editorState.options && editorState.options.lockHour ? 'disabled' : ''}>
          <span class="cs-time-colon">:</span>
          <input class="cs-input" type="number" min="0" max="59" step="5" data-cs-minute value="${(block.time || DEFAULT_TIME).split(':')[1]}" ${editorState.options && editorState.options.lockHour ? 'autofocus' : ''}>
        </div>
      </label>
      <label class="cs-field">
...
function wireEditor(pop, ouId) {
  ...
  // Time / duration
  const updateTime = () => {
    let h = pop.querySelector('[data-cs-hour]').value;
    let m = pop.querySelector('[data-cs-minute]').value;
    if (h) h = h.padStart(2, '0');
    if (m) m = m.padStart(2, '0');
    editorState.draft.blocks[0].time = `${h || '00'}:${m || '00'}`;
  };
  pop.querySelector('[data-cs-hour]').addEventListener('input', updateTime);
  pop.querySelector('[data-cs-minute]').addEventListener('input', updateTime);
